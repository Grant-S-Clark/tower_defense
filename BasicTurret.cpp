//Name: Grant Clark
//File: BasicTurret.cpp

#include "BasicTurret.h"

void BasicTurret::barrel_setup()
{
    //Make rectangle.
    //Grabs radius to get rectangle to edge, then gets
    //it to stick out by 10 pixels.
    barrels().push_back(
        sf::RectangleShape(sf::Vector2f(10, body().getRadius() + 15))
        );
    //Set colors.
...
void BasicTurret::aim_at(const Balloon & b)
{
...
std::vector< Bullet* > BasicTurret::fire()
{
    ...
    if (frame() == shoot_frame() && aiming())
    {
        ...
        float offset_y = sin(angle_radians) *
            barrels().at(0).getSize().y;
        ret.push_back(new BasicBullet(body().getPosition().x + offset_x,
